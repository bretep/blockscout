defmodule Explorer.ExchangeRates.TokenExchangeRates do
  ...
  def init(_args) do
    if Application.get_env(:explorer, ExchangeRates)[:enabled] do
      state = %__MODULE__{
        max_batch_size: Application.get_env(:explorer, __MODULE__)[:enabled] || @batch_size,
        interval: Application.get_env(:explorer, __MODULE__)[:enabled] || @interval
      }
...
# defmodule Explorer.ExchangeRates.TokenExchangeRatesRef do
#   @moduledoc """
